  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: T.bg }} edges={['top']}>
      ...
      >
        ...
        <View style={{
          ...
          {Platform.OS === 'web' && (
            <View style={{
              ...
              borderRadius: 18,
              // @ts-ignore
              background: 'radial-gradient(90% 120% at 100% 0%, rgba(42,111,219,0.10), transparent 60%)',
            }} />
          )}
        ...
        <Card style={{ overflow: 'hidden', marginBottom: 16 }}>
          {upNext.length === 0 ? (
            ...
            upNext.map((task, i) => (
              <TouchableOpacity
                key={task.id}
                style={{ flexDirection: 'row', alignItems: 'center', gap: 13, padding: 14, borderTopWidth: i > 0 ? 1 : 0, borderTopColor: T.line }}
                onPress={() => router.push(`/task/${task.id}`)}
              >
                <Checkbox done={task.done} color={task.done ? T.faint : T.indigo} onPress={() => toggleDone(task.id)} />
                <View style={{ flex: 1 }}>
                  <Text style={{ fontSize: 14.5, color: task.done ? T.faint : T.text, fontWeight: '500', fontFamily: FontFamily.sans, textDecorationLine: task.done ? 'line-through' : 'none' }} numberOfLines={1}>
                    {task.title}
                  </Text>
        ...
        {inboxCount > 0 && (
          <TouchableOpacity
            style={{ flexDirection: 'row', alignItems: 'center', gap: 12, backgroundColor: T.indigoBg, borderRadius: 18, borderWidth: 1, borderColor: T.indigoBd, padding: 16 }}
            onPress={() => router.push('/(tabs)/inbox')}
          >
            ...
            <View style={{ flex: 1 }}>
              ...
              <Mono color={T.indigo} size={9.5} spacing={1}>Tap to clarify with AI →</Mono>
            </View>
          </TouchableOpacity>
        )}
      </ScrollView>
